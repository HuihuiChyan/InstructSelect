...
OUTPUT_LABELS  = f"{INPUT_PREFIX}.lora8max_hdbscan.txt"
OUTPUT_VISUAL  = f"{INPUT_PREFIX}.lora8max_hdvisual.jsonl"
...
print("HDBSCAN clustering from scratch...")
km = HDBSCAN(min_cluster_size=30).fit(embeddings)
labels = km.labels_
...
if -1 in [k for k,v in counter.most_common()[:20]]:
    common = [k for k,v in counter.most_common()[:20]]
    common.remove(-1)
else:
    common = [k for k,v in counter.most_common()[:19]]
    
mappin = defaultdict(lambda: 20)
